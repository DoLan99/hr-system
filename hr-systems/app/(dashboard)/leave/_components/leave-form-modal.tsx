      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Ngày nghỉ</label>
              <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} required
                className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Số giờ nghỉ</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Loại nghỉ</label>
              ...
              {LEAVE_TYPES.map(t => (
                <button key={t.value} type="button" onClick={() => setForm(p => ({ ...p, type: t.value }))}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border-2 transition ${
                    form.type === t.value ? "border-blue-600 bg-blue-50 dark:bg-blue-950/40 text-blue-700" : "border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-blue-300"
                  }`}>
                  {t.label}
                </button>
              ))}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Lý do</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Link minh chứng (nếu có)</label>
