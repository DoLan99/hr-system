...
const VALUE_TYPES = [
  { value: "A", label: "A — Quan trọng", color: "border-red-500 bg-red-50 dark:bg-red-950/40 text-red-700" },
  { value: "B", label: "B — Trung bình", color: "border-yellow-500 bg-yellow-50 dark:bg-yellow-950/40 text-yellow-700" },
  { value: "C", label: "C — Thấp", color: "border-gray-400 bg-slate-50 dark:bg-slate-800/60 text-slate-600" },
];
...
export function MessageFormModal({ message, employees, customers, currentUserId, onClose, onSaved }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const supportVal = form.supportTime ? Number(form.supportTime) : undefined;
    const benefitVal = form.benefitTime ? Number(form.benefitTime) : undefined;
    if (supportVal !== undefined && supportVal < 0) { setError("Thời gian hỗ trợ không được âm"); return; }
    if (benefitVal !== undefined && benefitVal < 0) { setError("Thời gian có lợi không được âm"); return; }
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-lg my-4">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Kênh</label>
                ...
                {CHANNELS.map(c => (
                  <button key={c} type="button" onClick={() => set("channel", form.channel === c ? "" : c)}
                    className={cn("px-2 py-1 rounded text-xs font-medium border transition",
                      form.channel === c ? "border-blue-600 bg-blue-50 dark:bg-blue-950/40 text-blue-700" : "border-slate-200 dark:border-slate-700 text-slate-500")}>
                    {CHANNEL_LABELS[c]}
                  </button>
                ))}
              </div>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Chủ đề</label>
            <input value={form.subject} onChange={e => set("subject", e.target.value)} className={inputCls} placeholder="Subject / Tiêu đề..." />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Khách hàng</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Giao cho</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Tóm tắt nội dung</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Hành động cần làm</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Hạn xử lý</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-2">Mức độ giá trị</label>
              ...
              {VALUE_TYPES.map(v => (
                <button key={v.value} type="button" onClick={() => set("valueType", form.valueType === v.value ? "" : v.value)}
                  className={cn("flex-1 py-1.5 rounded-lg text-xs font-medium border-2 transition",
                    form.valueType === v.value ? v.color : "border-slate-200 dark:border-slate-700 text-slate-500")}>
                  {v.label}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Thời gian hỗ trợ (phút)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Thời gian có lợi (phút)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Link file</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Ghi chú follow-up</label>
            <textarea value={form.followUpNote} onChange={e => set("followUpNote", e.target.value)} rows={2}
              className={cn(inputCls, "resize-none")} placeholder="Ghi chú theo dõi tiếp theo..." />
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Câu trả lời của công ty</label>
            <textarea value={form.companyAnswer} onChange={e => set("companyAnswer", e.target.value)} rows={2}
              className={cn(inputCls, "resize-none")} placeholder="Phản hồi từ phía công ty..." />
          </div>
