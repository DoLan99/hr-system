...
const PAYMENT_TYPES = [
  { value: "SALARY", label: "Lương", color: "border-blue-500 bg-blue-50 text-blue-700" },
  { value: "BONUS", label: "Thưởng", color: "border-green-500 bg-green-50 text-green-700" },
  { value: "ADVANCE", label: "Tạm ứng", color: "border-yellow-500 bg-yellow-50 text-yellow-700" },
  { value: "DEDUCTION", label: "Khấu trừ", color: "border-red-500 bg-red-50 text-red-700" },
  { value: "OTHER", label: "Khác", color: "border-gray-500 bg-slate-50 text-slate-700" },
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-2">Loại thanh toán</label>
            <div className="flex flex-wrap gap-1.5">
              {PAYMENT_TYPES.map(t => (
                <button key={t.value} type="button" onClick={() => set("type", t.value)}
                  className={cn("px-3 py-1.5 rounded-lg text-xs font-medium border-2 transition",
                    form.type === t.value ? t.color : "border-slate-200 text-slate-600 hover:border-slate-300")}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Tháng lương (nếu có)</label>
