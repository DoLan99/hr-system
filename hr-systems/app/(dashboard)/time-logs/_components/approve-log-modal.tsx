      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          <div className="bg-slate-50 rounded p-3 text-sm">
            ...
            {log.note && <div className="text-xs text-slate-700 mt-2 italic">&ldquo;{log.note}&rdquo;</div>}
            {log.videoLink && (
              <a href={log.videoLink} target="_blank" rel="noopener" className="text-xs text-blue-600 hover:underline mt-2 inline-block">
                ▶ Xem video
              </a>
            )}
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-2 text-sm rounded ${mode === "approve" ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-600"}`}
            >
              Approve
            </button>
            <button
              onClick={() => setMode("reject")}
              className={`flex-1 py-2 text-sm rounded ${mode === "reject" ? "bg-red-600 text-white" : "bg-slate-100 text-slate-600"}`}
            >
              Reject
            </button>
          </div>
          ...
          {mode === "approve" ? (
            <>
              <div>
                <label className="text-xs text-slate-600 font-medium">Approved minutes</label>
                <input
                  type="number"
                  min={0}
              ...
              <div>
                <label className="text-xs text-slate-600 font-medium">Rating (1–5)</label>
                <input
                  type="number"
                  min={1}
                  max={5}
            ...
            <div>
              <label className="text-xs text-slate-600 font-medium">Lý do từ chối <span className="text-red-500">*</span></label>
              <textarea
