...
export function VaultFormModal({ vault, customers, employees, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-md my-4">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-3">
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Loại</label>
              ...
              {["COMPANY", "CUSTOMER"].map(s => (
                <button key={s} type="button" onClick={() => set("scope", s)}
                  className={cn("flex-1 py-2 rounded-lg text-xs font-medium border-2 transition",
                    form.scope === s ? "border-blue-600 bg-blue-50 dark:bg-blue-950/40 text-blue-700" : "border-slate-200 dark:border-slate-700 text-slate-600")}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Tên thực thể</label>
              <input value={form.entityName} onChange={e => set("entityName", e.target.value)} className={inputCls} placeholder="Google, GitHub..." />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Ứng dụng / Dịch vụ</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">URL đăng nhập</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Email dùng</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">2FA method</label>
              <input value={form.twoFaMethod} onChange={e => set("twoFaMethod", e.target.value)} className={inputCls} placeholder="TOTP, SMS..." />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Rotation (ngày)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Người phụ trách</label>
